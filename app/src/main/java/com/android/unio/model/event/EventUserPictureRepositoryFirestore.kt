...
class EventUserPictureRepositoryFirestore @Inject constructor(private val db: FirebaseFirestore) : EventUserPictureRepository {
    override fun addEventUserPicture(
        ...
        if (eventUserPicture.uid.isBlank()) {
            ...
            db.collection(EVENT_USER_PICTURES_PATH).document(eventUserPicture.uid).set(serialize(eventUserPicture))
        }
