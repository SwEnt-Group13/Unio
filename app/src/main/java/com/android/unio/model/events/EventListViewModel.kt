...
/**
 * ViewModel class that manages the event list data and provides it to the UI.
 * It uses an [EventRepository] to load the list of events and exposes them
 * through a [StateFlow] to be observed by the UI.
 *
...
class EventListViewModel(private val repository: EventRepository) : ViewModel() {
    ...
    /**
     * A private mutable state flow that holds the list of events.
     * It is internal to the ViewModel and cannot be modified from the outside.
     */
    private val _events = MutableStateFlow<List<Event>>(emptyList())

    /**
     * A public immutable [StateFlow] that exposes the list of events to the UI.
     * This flow can only be observed and not modified.
     */
     ...
     * This factory is used to create the ViewModel with the [EventRepositoryMock] dependency.
     */
    companion object {
        /**
