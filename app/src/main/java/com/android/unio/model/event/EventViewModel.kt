...
open class EventViewModel(private val repository: EventRepository, private val userRepository: UserRepository) : ViewModel() {
    ...
    fun saveEventForCurrentUser(eventUid: String, onSuccess: () -> Unit, onFailure: (Exception) -> Unit) {
        val currentUser = FirebaseAuth.getInstance().currentUser
        currentUser?.uid?.let { userUid ->
    ...
    fun unsaveEventForCurrentUser(eventUid: String, onSuccess: () -> Unit, onFailure: (Exception) -> Unit) {
        val currentUser = FirebaseAuth.getInstance().currentUser
        currentUser?.uid?.let { userUid ->
    ...
    fun isEventSavedForCurrentUser(eventUid: String, onResult: (Boolean) -> Unit) {
        val currentUser = FirebaseAuth.getInstance().currentUser
        currentUser?.uid?.let { userUid ->
            userRepository.isEventSaved(userUid, eventUid, onResult)
        } ?: run {
