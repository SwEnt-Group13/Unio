...
class MockAssociation {
    companion object {
        ...
        enum class EdgeCaseUrl(val value: String) {
            ...
            INVALID("https://assoc-with-invalid-url")
        }
        ...
        enum class EdgeCaseDescription(val value: String) {
            ...
            LONG("This is a very long association description intended to test the handling of large amounts of text. ".repeat(10)),
            SPECIAL_CHARACTERS("Description with special characters like #, @, $, %, and accents é, ü, ñ.")
        }

        enum class EdgeCaseImage(val value: String) {
            EMPTY(""),
            TYPICAL("https://example.com/image1.png"),
            LONG("https://example.com/very/long/path/to/image/that/may/exceed/length/limits/image12345.png"),
            INVALID("invalid-url")
        }
        ...
        fun createMockAssociation(
            ...
            val membersHelper = if (userDependency) {
                members
            } else {
                MockUser.createAllMockUsers(associationDependency = true, eventDependency = eventDependency)
            }
            return Association(
                uid = uid,
                url = url,
        ...
        fun createAllMockAssociations(eventDependency: Boolean = false, userDependency: Boolean = false, size: Int = 5): List<Association> {
            return List(size) { createMockAssociation(eventDependency = eventDependency, userDependency = userDependency) }
        }
    }
}
